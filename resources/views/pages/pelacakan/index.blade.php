...
new class extends Component
{
    ...
    #[Title('Lacak Antrean - Tangkab Melayani')]
    #[Layout('layouts.app')]
...
<div>
    ...
    <section class="bg-white">
        <div class="mx-auto max-w-2xl px-4 py-20 sm:px-6 lg:py-28">
            <div class="mb-10">
                <span class="text-xs font-semibold uppercase tracking-[0.15em] text-amber-600">Tracking</span>
            ...
            <form wire:submit="cari" class="flex gap-3">
                <div class="flex-1">
                    <input
                        type="text"
                        wire:model="nomor_antrean"
                        placeholder="Masukkan nomor antrean..."
                        class="w-full rounded-xl border border-neutral-200 bg-white px-4 py-3.5 text-sm text-neutral-900 placeholder-neutral-400 shadow-sm transition-all focus:border-amber-300 focus:outline-none focus:ring-2 focus:ring-amber-100"
                    >
                    @error('nomor_antrean')
                        <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                <button type="submit" class="shrink-0 rounded-xl bg-neutral-900 px-6 py-3.5 text-sm font-semibold text-white shadow-sm transition-all hover:bg-neutral-800 active:scale-[0.98]">
                    Cari
                </button>
            </form>
                        ...
                        <div class="rounded-2xl border border-neutral-200/60 bg-white p-6 sm:p-8 shadow-[0_4px_20px_-8px_rgba(0,0,0,0.06)]">
                            ...
                            <div class="flex items-center justify-between mb-6">
                                ...
                                <span class="inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-semibold ring-1 ring-inset {{ $badgeColors[$status] ?? 'bg-neutral-50 text-neutral-500 ring-neutral-600/20' }}">
                                    <span class="h-1.5 w-1.5 rounded-full {{ $status === 'waiting' ? 'bg-amber-500' : ($status === 'serving' ? 'bg-blue-500' : ($status === 'done' ? 'bg-emerald-500' : 'bg-neutral-400')) }}"></span>
                            ...
                            <div class="space-y-0">
                                ...
                                <div class="flex justify-between py-3 border-b border-neutral-100">
                                    ...
                                    <span class="text-sm font-medium text-neutral-900">{{ $result->created_at->format('d M Y, H:i') }}</span>
                                </div>
                                @if($result->waktu_dilayani)
                                <div class="flex justify-between py-3 border-b border-neutral-100">
                                    <span class="text-sm text-neutral-500">Dilayani</span>
                                    <span class="text-sm font-medium text-neutral-900">{{ $result->waktu_dilayani->format('d M Y, H:i') }}</span>
                                </div>
                                @endif
                                @if($result->waktu_selesai)
                                <div class="flex justify-between py-3 border-b border-neutral-100">
                                    <span class="text-sm text-neutral-500">Selesai</span>
                                    <span class="text-sm font-medium text-neutral-900">{{ $result->waktu_selesai->format('d M Y, H:i') }}</span>
                                </div>
                                @endif
                            </div>
                            ...
                            <div class="mt-6 pt-4 border-t border-neutral-100">
                                <a href="{{ route('tracking.show', $result->nomor_antrean) }}" wire:navigate class="inline-flex items-center gap-1.5 text-sm font-medium text-amber-600 hover:text-amber-700 transition-colors">
