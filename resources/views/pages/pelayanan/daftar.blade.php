...
<div>
    ...
    <section class="bg-neutral-50/60 min-h-screen">
        <div class="mx-auto max-w-2xl px-4 py-16 sm:px-6 lg:py-20">
            ...
            <a href="{{ route('pelayanan.index') }}" wire:navigate class="inline-flex items-center gap-1.5 text-sm text-neutral-500 hover:text-amber-600 transition-colors mb-6">
            ...
            @if($success)
                {{-- Success Screen --}}
                <div class="rounded-2xl border border-neutral-200/60 bg-white p-8 sm:p-12 text-center shadow-[0_4px_20px_-8px_rgba(0,0,0,0.06)]">
                    ...
                    <span class="text-xs font-semibold uppercase tracking-[0.15em] text-amber-600">Sukses</span>
                    ...
                    <div class="mt-8 inline-block rounded-2xl border-2 border-amber-200/60 bg-amber-50/50 px-8 py-5">
                        <p class="text-xs text-amber-600 font-medium uppercase tracking-wider">Nomor Antrean</p>
                        <p class="mt-1 text-3xl sm:text-4xl font-bold font-mono text-amber-700 tracking-wider">{{ $nomor_antrean }}</p>
                    </div>
                    ...
                    <div class="mt-6 rounded-xl border border-neutral-100 bg-neutral-50/50 p-5 text-left text-sm text-neutral-500">
                        ...
                        <ul class="space-y-1.5">
                            <li class="flex items-start gap-2">
                                <svg class="h-4 w-4 text-amber-500 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                <span>Antrean Anda saat ini berstatus <strong class="text-amber-600">Menunggu</strong></span>
                            </li>
                            <li class="flex items-start gap-2">
                                <svg class="h-4 w-4 text-amber-500 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            ...
            @else
                {{-- Form --}}
                <div class="mb-8">
                    <span class="text-xs font-semibold uppercase tracking-[0.15em] text-amber-600">Pendaftaran</span>
                ...
                <div class="rounded-2xl border border-neutral-200/60 bg-white p-6 sm:p-8 shadow-[0_4px_20px_-8px_rgba(0,0,0,0.06)]">
                    <form wire:submit="daftar" class="space-y-5">
                                ...
                                @switch($field->type)
                                    @case('textarea')
                                        <textarea
                                            ...
                                            rows="3"
                                            class="w-full rounded-xl border border-neutral-200 bg-white px-4 py-3 text-sm text-neutral-900 placeholder-neutral-400 shadow-sm transition-all focus:border-amber-300 focus:outline-none focus:ring-2 focus:ring-amber-100"
                                        ></textarea>
                                        @break
                                    ...
                                    @case('select')
                                        <select
                                            wire:model="{{ $wireModel }}"
                                            class="w-full rounded-xl border border-neutral-200 bg-white px-4 py-3 text-sm text-neutral-900 shadow-sm transition-all focus:border-amber-300 focus:outline-none focus:ring-2 focus:ring-amber-100"
                                        >
                                    ...
                                    @case('image')
                                        <div class="relative">
                                            <input
                                                ...
                                                class="block w-full text-sm text-neutral-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-amber-50 file:text-amber-700 hover:file:bg-amber-100 transition-all cursor-pointer"
                                            >
                                        </div>
                                        @if($field->type === 'image' && !empty($uploads[$field->name]))
                                            <div class="mt-3">
                                                <img src="{{ $uploads[$field->name]->temporaryUrl() }}" class="h-24 w-24 rounded-xl object-cover border border-neutral-200">
                                            </div>
                                        @endif
                                        @break
                        ...
                        <div class="pt-4 border-t border-neutral-100">
                            <button type="submit" class="w-full rounded-xl bg-neutral-900 px-6 py-3.5 text-sm font-semibold text-white shadow-sm transition-all hover:bg-neutral-800 active:scale-[0.99]">
                                Ambil Antrean
                            </button>
