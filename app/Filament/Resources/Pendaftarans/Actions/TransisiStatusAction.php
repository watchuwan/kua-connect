...
class TransisiStatusAction
{
        ...
        if ($target === StatusPendaftaran::PerluRevisi) {
            $action = $action->schema([
                Textarea::make("catatan")
                    ...
                    ->placeholder(
                        "Jelaskan bagian apa saja yang perlu diperbaiki...",
                    )
                    ->required()
                    ->rows(3),
            ]);
            ...
            $action = $action->schema(
                fn(Pendaftaran $record): array => static::getFormSchema(
                    $record,
                    $target,
                ),
            );
    ...
    public static function getFormSchema(
        Pendaftaran $record,
        StatusPendaftaran $target,
    ): array {
        if (
            $target === StatusPendaftaran::VerifikasiFisik &&
            $record->pelayanan?->slug === "pendaftaran-nikah"
        ) {
            return [
        ...
        if (
            $target === StatusPendaftaran::JadwalDikunci &&
            $record->pelayanan?->slug === "pendaftaran-nikah"
        ) {
            return [
        ...
        if (
            $target === StatusPendaftaran::SiapIkrar &&
            $record->pelayanan?->slug === "sertifikasi-wakaf"
        ) {
            return [
                DateTimePicker::make("jadwal_kedatangan")
                    ->label("Jadwal Ikrar")
        ...
        if (
            $target === StatusPendaftaran::JadwalDitugaskan &&
            $record->pelayanan?->slug === "kalibrasi-arah-kiblat"
        ) {
            return [
                DateTimePicker::make("jadwal_kedatangan")
        ...
        if (
            $target === StatusPendaftaran::Selesai &&
            $record->pelayanan?->slug === "kalibrasi-arah-kiblat"
        ) {
            return [
    ...
    public static function transisiStatus(
        Pendaftaran $record,
        StatusPendaftaran $target,
        array $data = [],
    ): void {
